...
// Envuelve cualquier precio ya formateado con la etiqueta "MXN" en
// gris pequeño, para dejar claro que no son dólares (catálogo de
// importación americana). Solo cambia lo que se muestra — no toca
// price ni ningún cálculo.
function withMXN(precioTexto) {
  return precioTexto + ' <span class="mxn-tag">MXN</span>';
}
...
function priceBlockHTML(p) {
  ...
  if (!pct) {
    return '<div class="price-block"><div class="price-row"><span class="price-current">' + withMXN(p.price) + '</span></div></div>';
  }

  var finalPrice = parsePriceNumber(displayPrice(p));
  var html = '<div class="price-block">';
  html +=   '<div class="price-original">' + withMXN(p.price) + '</div>';
  html +=   '<div class="price-row">';
  html +=     '<span class="price-current">' + withMXN(displayPrice(p)) + '</span>';
  html +=     '<span class="price-badge">' + pct + '% OFF</span>';
  html +=   '</div>';
  if (p.meses) {
    html += '<div class="price-installments">' + p.meses + ' meses de ' + withMXN(formatMXN(calcularMensualidad(finalPrice, p.meses))) + '</div>';
  }
  html += '</div>';
  return html;
}
